...
function Registration() {
  ...
  function handleRegistration(e){
    ...
    if (!password){
      ...
      setErrorPassword("Password mush be at least 6 characters")
    }
    if (!rePassword){
      setErrorRePassword("Confirm your password")
    }else if(rePassword !== password){
      setErrorRePassword("Password not matches")
    }

    if(clientName && email && emailValidation(email) && password && password.length >= 6 && rePassword && rePassword === password){
      setClientName("")
      seEmail("")
      setPassword("")
      setRePassword("")
  ...
  return (
    <div className="w-full">
      <div className="bg-gray-100 pb-10 ">
        <form onSubmit={handleRegistration} className="flex items-center flex-col w-[370px] mx-auto ">
          ...
          <div className="border-zinc-200 p-6 border w-full">
            ...
            <div className="flex flex-col gap-3">
              <div className="flex flex-col gap-2">
                ...
                value={clientName}
                onChange={(e)=>{ setClientName(e.target.value); setErrorClientName("")}}
                />
                {errorClientName && (
                  <p className="text-red-600 text-xs font-semibold tracking-wide flex items-center gap-2 -mt-1.5">
                    <span className="italic font-extrabold font-titleFont text-base">!</span>{errorClientName}
                  </p>
                )}
              </div>
              <div className="flex flex-col gap-2">
                ...
                onChange={(e)=>{ seEmail(e.target.value) ; setErrorEmail("")}}
              ...
              <div className="flex flex-col gap-2">
                ...
                className="w-full py-1 border border-zinc-400 px-2 text-balance rounded-sm outline-none focus:border-[#e77600] focus:shadow-amazonInput duration-100 "    />
                {errorPassword && (
                  <p className="text-red-600 text-xs font-semibold tracking-wide flex items-center gap-2 -mt-1.5">
              ...
              <div className="flex flex-col gap-2">
                ...
                <input type="password"
                value={rePassword}
                onChange={(e)=> {setRePassword(e.target.value); setErrorRePassword("")}}
                className="w-full  py-1 border border-zinc-400 px-2 text-balance rounded-sm outline-none focus:border-[#e77600] focus:shadow-amazonInput duration-100 "    />
                {errorRePassword && (
                  <p className="text-red-600 text-xs font-semibold tracking-wide flex items-center gap-2 -mt-1.5">
                    <span className="italic font-extrabold font-titleFont text-base">!</span>{errorRePassword}
                  </p>
                )}
